...
### 3.1 Transpiler AST & Lexing Robustness
- **Power Operator (`^`) Transformation**: BASIC expressions of the form `a ^ b` are converted to `sycl::pow(a, b)` or `sycl::exp(b)` (when base is `e`). The transpiler scans backward and forward from `^`, tracking nested parenthesis depths and identifier boundaries (including function calls like `act("H+")^0.5` and qualified identifiers).
- **Whitespace Tolerance**: Scans skip whitespace before and after operators to handle spaced forms like `10 ^ -3.19` or `e ^ ( -Ea / R * deltaT )`.
- **Branch Condition Conversion**: Translates `IF <cond> THEN GOTO <line>` into structured C++ `if (<cond>) { goto L<line>; }`, replacing BASIC logical operators `AND` $\rightarrow$ `&&`, `OR` $\rightarrow$ `||`, and preserving relational operators.

---

## 4. Kernel Specialization & Compilation Pipeline

The `JitCompiler` generates a unique `.cpp` source file in `.cache/` containing all transpiled rate functions, compiles it into a shared object (`.so`) using the AdaptiveCpp compiler driver, and dynamically loads it.

### 4.1 Compiler Driver & Build Integration
- **Compiler Command**: Uses `acpp -O3 -shared -fPIC -I<include_dir> --acpp-targets=generic --acpp-export-all <cpp_path> -o <so_path>`.
- **CMake Macro Support**: To support embedding into parent projects (such as `POET-MP`), CMake defines compile definitions `LITEPHREEQC_INCLUDE_DIR` and `LITEPHREEQC_SRC_DIR`. When defined, the JIT compiler resolves headers and template definitions from absolute source paths, preventing compilation failures when the application's working directory varies at runtime.

### 4.2 Registration & SSCP Reflection
The generated code registers transpiled functions into the singleton `RateRegistry`:

```cpp
// Auto-generated in .cache/solver_<phases>.cpp
